...
class TestimoniController extends Controller
{
    public function prosesTestimoni (Request $request, $id) {
        ...
        $notif = Notifikasi::where('user_id', $id)
                ->insert(['link' => route('profilUser', ['user_id' => $id]), 'user_id' => $id, 'status' => $request->input('status'), 'nama' => $request->input('nama'), 'verb' => $request->input('verb'), 'isiNotif' => $request->input('isiTestimoni')]);
